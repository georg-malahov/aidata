angular.module('app').controller('TableController',
    [
        '$scope', '$rootScope', '$filter', '$timeout', '$interpolate', '$window',
        'ColumnsService',
        ($scope, $rootScope, $filter, $timeout, $interpolate, $window, ColumnsService) ->
            ...
            $scope.gridOptions =
                ...
                onRegisterApi: (gridApi) ->
                    ...
                    if count <= 50
                        angular.element(document.getElementsByClassName('tablegrid_grid')[0]).css('height',
                            count * 30 + 45 + 'px'
                        )
                    else
                        angular.element(document.getElementsByClassName('tablegrid_grid')[0]).css('height',
                    ...
                    $timeout(() ->
                        angular.element(document.getElementsByClassName('ui-grid-canvas')[1]).css('height',
                            'auto'
                        )
                    , 100
                    )
